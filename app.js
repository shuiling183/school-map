// v20 - editable fields + edit history + community outlines
var VERSION='v20',map,favIds,hiddenIds,showFavOnly,currentSchool;

// Login
function doLogin(){if(document.getElementById('pwdInput').value==='shanghai2026'){localStorage.setItem('auth20','1');document.getElementById('loginPage').style.display='none';document.getElementById('mapPage').style.display='flex';setTimeout(initMap,200);}else document.getElementById('loginError').style.display='block';}
if(localStorage.getItem('auth20')==='1'){document.getElementById('loginPage').style.display='none';document.getElementById('mapPage').style.display='flex';setTimeout(initMap,300);}

// Edit history
function getHistory(){try{return JSON.parse(localStorage.getItem('hist20')||'[]');}catch(e){return[];}}
function addHistory(school,changes){var h=getHistory();h.unshift({time:new Date().toLocaleString(),name:school.name,changes:changes});if(h.length>200)h=h.slice(0,200);localStorage.setItem('hist20',JSON.stringify(h));}

// Data helpers
function getEdits(){try{return JSON.parse(localStorage.getItem('ed20')||'{}');}catch(e){return{};}}
function setEdits(e){localStorage.setItem('ed20',JSON.stringify(e));}
function getGeo(){try{return JSON.parse(localStorage.getItem('g20')||'{}');}catch(e){return{};}}
function setGeo(c){localStorage.setItem('g20',JSON.stringify(c));}
function sd(s){var e=getEdits(),d=e[s.id]||{};return{
  ...
  sr25:d.sr25!==undefined?d.sr25:s.srRate25,
  sr24:d.sr24!==undefined?d.sr24:s.srRate24,
  sr23:d.sr23!==undefined?d.sr23:s.srRate23,
  res25:d.res25!==undefined?d.res25:s.residency25,
  hl:d.hl!==undefined?d.hl:s.highlights,
...
// Community outlines
var communityCircles=[];
function clearCommunityOutlines(){for(var i=0;i<communityCircles.length;i++){communityCircles[i].setMap(null);}communityCircles=[];}
function drawCommunities(school){
  clearCommunityOutlines();
  var d=sd(school),text=d.communities||'';
      ...
      gc.getLocation(name,function(status,result){
        if(status==='complete'){
          if(result.geocodes&&result.geocodes.length===1){
            ...
            drawCommunityCircle(loc.lng,loc.lat,name);
          }else if(result.geocodes&&result.geocodes.length>1){
            // Multiple results - pick first and note
            var loc=result.geocodes[0].location;
            drawCommunityCircle(loc.lng,loc.lat,name+' (近似)');
          }
        }
      });
    })(parts[i]);
...
function drawCommunityCircle(lng,lat,name){
  var circle=new AMap.CircleMarker({center:[lng,lat],radius:15,fillColor:'#2ecc71',fillOpacity:0.2,strokeColor:'#2ecc71',strokeWeight:2,strokeOpacity:0.6});
  circle.setMap(map);
  communityCircles.push(circle);
  var label=new AMap.Text({text:name,position:[lng,lat],offset:new AMap.Pixel(0,-20),style:{'font-size':'9px','color':'#2ecc71','background':'rgba(255,255,255,0.8)','padding':'1px 3px','border-radius':'2px'},zIndex:90});
  label.setMap(map);
  communityCircles.push(label);
}

// Init
function initMap(){
  favIds=new Set(JSON.parse(localStorage.getItem('fv20')||'[]'));
  hiddenIds=new Set(JSON.parse(localStorage.getItem('hd20')||'[]'));
  showFavOnly=false;
...
// Render
function doRender(){
  if(!map)return;map.clearMap();clearCommunityOutlines();
  ...
  for(var i=0;i<ALL_SCHOOLS.length;i++){
    var s=ALL_SCHOOLS[i];
    if(df&&s.district!==df)continue;var tier=sd(s).tier||'';
  ...
  for(i=0;i<list.length;i++){
    var s=list[i],c=getCoord(s);if(!c)continue;
    ...
    var cm=new AMap.CircleMarker({center:[c.lng,c.lat],radius:r,fillColor:fc,fillOpacity:0.9,strokeColor:'#fff',strokeWeight:2});cm.setMap(map);
  ...
  document.getElementById('schoolCount').innerHTML='共 '+list.length+' 所 ('+VERSION+') <a href="#" onclick="startGeo()" style="color:#2ecc71">定位</a> <a href="#" onclick="localStorage.removeItem(\'g20\');location.reload()" style="color:#e94560">重置</a> <a href="#" onclick="showHistory()" style="color:#f39c12">历史</a>';
}
...
// Detail panel - now with all editable fields
function showDetail(s){
  ...
  var h='<button onclick="document.getElementById(\'detailPanel\').style.display=\'none\';clearCommunityOutlines();" style="float:right;background:transparent;color:#aaa;font-size:16px;cursor:pointer;">X</button>';
  h+='<h3>'+s.name+' <span class="tier-tag '+tc+'">'+tier+'</span></h3>';
  h+='<p>全称: '+(s.fullName||'-')+' | '+s.district+' | '+(s.type||'')+'</p>';
  h+='<p>地址: '+(s.address||'-')+'</p>';
  h+='<hr><b style="color:#e94560">可编辑字段</b><br>';
  h+=erow('梯队','tier_'+s.id,tier,'如: 一梯队 / 二梯队 / 三梯队');
  h+=erow('市重率2025','sr25_'+s.id,t.sr25,'如: 约40%');
  h+=erow('市重率2024','sr24_'+s.id,t.sr24,'');
  h+=erow('市重率2023','sr23_'+s.id,t.sr23,'');
  h+=erow('入户年限2025','res25_'+s.id,t.res25,'如: 户籍满3年');
  h+=erow('中签率','lottery_'+s.id,t.lottery,'仅民办填写');
  h+=erow('对口小学','feeder_'+s.id,t.feeder,'多个小学用逗号分隔');
  ...
  h+='<p>备注:</p><textarea id="notes_'+s.id+'" style="width:100%;height:50px">'+t.notes.replace(/"/g,'&quot;')+'</textarea>';
  ...
  h+='<button onclick="drawCommunities(ALL_SCHOOLS.find(function(x){return x.id==='+s.id+';}))" style="background:#3498db">框选小区</button> ';
  h+='<button onclick="document.getElementById(\'detailPanel\').style.display=\'none\';clearCommunityOutlines();">关闭</button>';
  h+='</div>';
  document.getElementById('detailContent').innerHTML=h;
  document.getElementById('detailPanel').style.display='block';

  // Auto-draw communities if data exists
  drawCommunities(s);
}
function erow(label,id,val,placeholder){
  return '<div style="margin:2px 0;font-size:12px">'+label+': <input id="'+id+'" value="'+(val||'').replace(/"/g,'&quot;')+'" placeholder="'+(placeholder||'')+'" style="width:100%;padding:3px 6px;background:#16213e;color:#eee;border:1px solid #444;border-radius:3px;font-size:12px"></div>';
}

// Save with history
function saveEdit(id){
  var oldE=getEdits();var oldD=oldE[id]||{};
  ...
  if(s){
    var changes=[];
    for(var k in newD){if(newD[k]!==(oldD[k]||''))changes.push(k+': '+(oldD[k]||'无')+' → '+newD[k]);}
    if(changes.length>0)addHistory(s,changes.join('; '));
  }
...
function showHistory(){
  var h=getHistory();if(h.length===0){alert('暂无编辑记录');return;}
  var text='最近编辑记录:\n\n';
  for(var i=0;i<Math.min(h.length,30);i++){text+=h[i].time+' | '+h[i].name+'\n  '+h[i].changes+'\n\n';}
  alert(text);
}

// 收藏/隐藏/筛选
function toggleFav(id){if(favIds.has(id))favIds.delete(id);else favIds.add(id);localStorage.setItem('fv20',JSON.stringify(Array.from(favIds)));doRender();if(currentSchool&&currentSchool.id===id)showDetail(currentSchool);}
function toggleHide(id){if(hiddenIds.has(id))hiddenIds.delete(id);else hiddenIds.add(id);localStorage.setItem('hd20',JSON.stringify(Array.from(hiddenIds)));doRender();if(currentSchool&&currentSchool.id===id)showDetail(currentSchool);}
...
function exportCSV(){
  ...
  for(var i=0;i<ALL_SCHOOLS.length;i++){
    ...
    csv+=['"'+s.district+'"','"'+s.name+'"','"'+s.type+'"','"'+(s.fullName||'')+'"','"'+d.lottery+'"','"'+(s.residency23||'')+'"','"'+(s.residency24||'')+'"','"'+d.res25+'"','"'+d.feeder+'"','"'+d.communities+'"','"'+d.sr23+'"','"'+d.sr24+'"','"'+d.sr25+'"','"'+d.tier+'"','"'+d.hl+'"','"'+(s.address||'')+'"'].join(',')+'\n';
  }
...
function exportHistory(){
  var h=getHistory();if(h.length===0){alert('无记录');return;}
  var csv='时间,学校,变更内容\n';
  for(var i=0;i<h.length;i++){csv+='"'+h[i].time+'","'+h[i].name+'","'+h[i].changes+'"\n';}
  var b=new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8'});var a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='编辑历史.csv';a.click();
}
